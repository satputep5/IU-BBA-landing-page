    ...
    >
      <div className="flex flex-col md:flex-row space-x-8 w-full max-w-[1200px] overflow-x-hidden py-2 md:py-4 justify-center transition-all duration-500 ease-in-out px-8">
        {groupedTestimonials[activeDot]?.map((testimonial, index) => (
          <div
            key={index}
            className="bg-white p-6 rounded-lg flex flex-col sm:flex-row w-full max-w-[800px] mx-4 flex-shrink-0 transition-all duration-300 ease-in-out border border-[#FACB6E] hover:shadow-xl hover:border-2 hover:border-[#FACB6E]"
            style={{
              boxShadow: "0 2px 4px rgba(250, 203, 110, 0.4)",
            }}
          >
        ...
        {groupedTestimonials.map((_, index) => (
          <button
            ...
            className={`h-3 w-3 rounded-full mx-1 ${
              activeDot === index ? "bg-[#FACB6E]" : "bg-gray-300"
...
function App() {
  return (
    <div className="App">
      <h1 className="text-3xl sm:text-4xl font-bold text-center pt-4 text-[#FACB6E]">
        What Our Alumni Say
      </h1>
      <Testimonials />
    </div>
